...
import {
  playXO, playGuess, playRPS, playDice
} from './games.js'
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    try {
      ...
      if (matchCommand(text, COMMANDS.ping)) {
        ...
        let animeUrl = null
        try { animeUrl = getAnimeImage('neko') } catch (e) {}
        if (animeUrl) {
...
𝑩𝑶𝑻 𝑫𝑨𝑹𝑲 © 2026`
        ...
        let animeUrl = null
        try { animeUrl = getAnimeImage('waifu') } catch (e) {}
...
╰━━━━━━━━━━━━━━━╯`
      ...
      // ═══ الألعاب ═══
      if (matchCommand(text, COMMANDS.xo)) {
        return playXO(sock, from, msg, senderJid, senderNum)
      }
      if (matchCommand(text, COMMANDS.guess)) {
        return playGuess(sock, from, msg, senderJid, senderNum)
      }
      if (matchCommand(text, COMMANDS.rps)) {
        return playRPS(sock, from, msg, senderJid, senderNum)
      }
      if (matchCommand(text, COMMANDS.dice)) {
        return playDice(sock, from, msg, senderJid, senderNum)
      }
      ...
      if (text.trim().startsWith('تشغيل') || text.trim().startsWith('شغل ') || text.trim().startsWith('play ')) {
        ...
        if (query.length >= 2) {
          return playMusic(sock, from, msg, query)
        }
      }
      ...
      if (text.trim().startsWith('بحث') || text.trim().startsWith('search ')) {
        ...
        if (query.length >= 2) {
          return searchMusic(sock, from, msg, query)
        }
      }
      ...
      if (text.trim().startsWith('ذكاء') || text.trim().startsWith('ai ') || text.trim().startsWith('اسأل')) {
        ...
        if (question.length >= 2) {
          return askAI(sock, from, msg, question)
        }
      }
      ...
      if (text.trim().startsWith('صوره') || text.trim().startsWith('image ') || text.trim().startsWith('img ')) {
        ...
        if (prompt.length >= 2) {
          return generateImage(sock, from, msg, prompt)
        }
      }
      ...
      // ═══ اقتباس ═══
      if (matchCommand(text, COMMANDS.quote)) {
        return randomQuote(sock, from, msg)
      }

      // ═══ نكتة ═══
      if (matchCommand(text, COMMANDS.joke)) {
        return randomJoke(sock, from, msg)
      }

      // ═══ هل تعلم ═══
      if (matchCommand(text, COMMANDS.fact)) {
        return randomFact(sock, from, msg)
      }

      // ═══ فضح ═══
      if (matchCommand(text, COMMANDS.expose) && isGroup) {
        return exposeMedia(sock, from, msg, senderJid)
      }
      ...
      if (isGroup) {
        ...
        if (matchCommand(text, COMMANDS.kick) && userIsAdmin && botIsAdmin) {
          return kickMember(sock, from, msg, getMentioned(msg))
        }
        if (matchCommand(text, COMMANDS.promote) && userIsAdmin && botIsAdmin) {
          return promoteMember(sock, from, msg, getMentioned(msg))
        }
        if (matchCommand(text, COMMANDS.demote) && userIsAdmin && botIsAdmin) {
          return demoteMember(sock, from, msg, getMentioned(msg))
        }
        if (matchCommand(text, COMMANDS.mute) && userIsAdmin && botIsAdmin) {
          return muteGroup(sock, from, msg)
        }
        if (matchCommand(text, COMMANDS.unmute) && userIsAdmin && botIsAdmin) {
          return unmuteGroup(sock, from, msg)
        }
        if (matchCommand(text, COMMANDS.tagall) && userIsAdmin) {
          return tagAll(sock, from, msg, '')
        }
        if (matchCommand(text, COMMANDS.groupLink) && userIsAdmin && botIsAdmin) {
          return getGroupLink(sock, from, msg)
        }
        if (matchCommand(text, COMMANDS.revoke) && userIsAdmin && botIsAdmin) {
