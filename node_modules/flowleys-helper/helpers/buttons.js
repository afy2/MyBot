...
async function sendUrlButton(sock, jid, displayText, url) {
...
function logSendCards(label, data) {
  console.log(`\n📤 sendCards :: ${label}`)
  console.dir(sanitizeForLog(data), { depth: null })
}


async function sendCards(sock, jid, content = {}, options = {}) {
  ...
  const carouselCards = await Promise.all(
    cards.map(async (card) => {
      ...
      if (card.image) {
        ...
        throw new Error('Each card must have an image or video')
      }
      ...
      if (Buffer.isBuffer(media)) {
        ...
        throw new Error('Media must be Buffer, URL string, or { url }')
      }
  ...
  const hasBodyText = typeof text === 'string' && text.trim() !== ''

  if (hasBodyText) {
    interactivePayload = proto.Message.InteractiveMessage.create({
      body: { text },
  ...
  // 🧪 LOGS
  // =============================
  logSendCards('interactivePayload', interactivePayload)
  ...
  logSendCards('generatedMessage', msg)

  await sock.relayMessage(jid, msg.message, {
    messageId: msg.key.id
  })

  logSendCards('relayMessage', {
    jid,
    messageId: msg.key.id
  })
